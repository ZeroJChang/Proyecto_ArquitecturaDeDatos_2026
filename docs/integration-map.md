...
- **Spark pipeline down:** Kafka retains frames within the topic retention window. On restart, each pipeline resumes from its checkpointed offset, so no committed frame is skipped or double-written (see [ADR-0004](adrs/0004-spark-checkpointing.md)).
...
> Note: with a single Kafka broker and replication factor 1 (current local/demo setup), an unrecovered broker disk loss would lose unconsumed frames. The 5-year scaling plan raises replication to 3 — see [ADR-0003](adrs/0003-kafka-kraft-broker.md) and [`hld.md`](hld.md).
