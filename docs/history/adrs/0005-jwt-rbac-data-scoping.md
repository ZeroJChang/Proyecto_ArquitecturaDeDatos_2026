...
Use **stateless, HMAC-signed JWTs** issued at [Login](../../flows/login/index.md) carrying claims `{ sub, email, role, branchId }`. Authorization is enforced in two layers: `@Roles(...)` + `RolesGuard` gate endpoints by role, and each query handler applies **data scoping** — branch operators are filtered to their `branchId`, owners to vehicles linked in `vehicle_owners`.
...
- Scoping bugs are confidentiality bugs; handlers like [Query GPS Events](../../flows/query-gps-events/index.md) and [Query Status Events](../../flows/query-status-events/index.md) verify ownership/branch before returning data.
